...
species_info_ui <- function(id) {
    ...
    tagList(grid(grid_template = vboxGrid,
                 ...
                 info1 = card(class = "card_cls", uiOutput(ns("species_info"))),
                 info2 = card(class = "card_cls", uiOutput(ns("taxonomy_info"))),
                 info3 = card(class = "card_cls", uiOutput(ns("geo_info")))
    ))
}
...
species_info_server <- function (input, output, session, diversity_dt) {
    ...
    output$species_info <- renderUI({
        ...
        tagList(div(class="content",
                    div(class="header", div("Species", shiny.semantic::icon("paw"))),
                    div(class="description", info_text)
        ))
    ...
    output$taxonomy_info<- renderValueBox({
        ...
        tagList(div(class="content",
                    div(class="header", div("Taxonomy", shiny.semantic::icon("list"))),
                    div(class="description", info_text)
        ))
    })
    ...
    output$geo_info <- renderValueBox({
        ...
        info_text <- HTML(glue("<div class='info_cls'><span><b>Countinent:</b>{countinent_name}</span><br><span><b>Country:</b>{country_name}</span></div>"))
        
        tagList(div(class="content",
                    div(class="header", div("Geographic", shiny.semantic::icon("map marked"))),
                        div(class="description", info_text)
                    ))
    })
}
